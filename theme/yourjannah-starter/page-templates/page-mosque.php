...
<main class="ynj-main">
  <div class="ynj-desktop-grid">
    ...
    <?php if ( $mosque && function_exists( 'ynj_get_league_standings' ) ) :
        $_ldata = ynj_get_league_standings( (int) $mosque->id, $mosque->city ?? null, 7 );
        $_cp_bar = function_exists( 'ynj_get_congregation_points' ) ? ynj_get_congregation_points( (int) $mosque->id, 7 ) : null;
        $_fajr_bar = function_exists( 'ynj_fajr_counter' ) ? ynj_fajr_counter( (int) $mosque->id ) : 0;
    ?>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;">
            ...
            <div style="display:flex;align-items:center;gap:12px;font-size:12px;">
                ...
                <span style="text-align:center;">
                    <strong style="font-size:16px;display:block;"><?php echo number_format( $_cp_bar['prayers']['total'] ); ?></strong>
                    <span style="font-size:9px;opacity:.6;"><?php esc_html_e( 'prayers', 'yourjannah' ); ?></span>
                </span>
                <?php endif; ?>
                <?php if ( $_cp_bar && $_cp_bar['quran_pages'] > 0 ) : ?>
                <span style="text-align:center;">
                    <strong style="font-size:16px;display:block;"><?php echo number_format( $_cp_bar['quran_pages'] ); ?></strong>
                    <span style="font-size:9px;opacity:.6;"><?php esc_html_e( 'pages', 'yourjannah' ); ?></span>
                </span>
                <?php endif; ?>
                <?php if ( $_ldata['rank'] > 0 ) : ?>
                <span style="background:rgba(255,255,255,.15);padding:6px 10px;border-radius:8px;text-align:center;">
                    <strong style="font-size:16px;display:block;">#<?php echo $_ldata['rank']; ?></strong>
                    <span style="font-size:9px;opacity:.6;"><?php esc_html_e( 'rank', 'yourjannah' ); ?></span>
                </span>
                <?php endif; ?>
            </div>
        </div>
    ...
    <?php if ( $mosque && function_exists( 'ynj_get_league_standings' ) ) :
        ...
        if ( $league['total'] > 0 ) :
    ?>
    <section class="ynj-card" id="mosque-league-table" style="padding:0;overflow:hidden;border:1px solid #fde68a;">
        <!-- League Header -->
        ...
        <div style="padding:0;">
            ...
            <div style="display:flex;align-items:center;gap:10px;padding:10px 16px;border-bottom:1px solid #f5f0e8;<?php echo $is_me ? 'background:#fef3c7;' : ''; ?>">
                <span style="font-size:16px;min-width:24px;text-align:center;font-weight:800;color:<?php echo $pos <= 3 ? '#92400e' : '#a16207'; ?>;">
                    <?php echo $medals[ $i ] ?? $pos; ?>
                </span>
        ...
        <!-- Your Breakdown (if ranked) -->
        <?php if ( $league['rank'] > 0 && ! empty( $league['breakdown'] ) ) :
            $bd = $league['breakdown'];
        ?>
