<?php
/**
 * Seed well-known mosques with comprehensive data (announcements, events, sponsors, etc.).
 * Looks up mosques by slug — works regardless of auto-increment IDs.
 * Run: wp eval-file wp-content/plugins/yn-jannah/seed-full.php
 * Or: ?ynj_seed_full=1 from WP admin
 */
...
// Target mosques by SLUG — resolve IDs dynamically
$slug_targets = [
    'the-london-central-mosque-algame-fund' => [ 'name' => 'London Central Mosque', 'city' => 'London', 'address' => '146 Park Road, Regent\'s Park', 'capacity' => 5000, 'has_women_section' => 1, 'has_parking' => 1 ],
    'east-london-mosque-trust'               => [ 'name' => 'East London Mosque', 'city' => 'London', 'address' => '82-92 Whitechapel Road', 'capacity' => 7000, 'has_women_section' => 1, 'has_parking' => 0 ],
    'birmingham-central-mosque'              => [ 'name' => 'Birmingham Central Mosque', 'city' => 'Birmingham', 'address' => '180 Belgrave Middleway, Highgate', 'capacity' => 6000, 'has_women_section' => 1, 'has_parking' => 1 ],
    'manchester-central-mosque-and-islamic-cultural-centre' => [ 'name' => 'Manchester Islamic Centre', 'city' => 'Manchester', 'address' => 'Upper Park Road, Victoria Park', 'capacity' => 3000, 'has_women_section' => 1, 'has_parking' => 1 ],
];

// Also include the demo mosque (ID 1 fallback)
$demo_id = (int) $wpdb->get_var( "SELECT id FROM {$p}mosques WHERE slug = 'yourniyyah-masjid' LIMIT 1" );
if ( ! $demo_id ) $demo_id = 1;

$targets = [];
foreach ( $slug_targets as $slug => $data ) {
    $id = (int) $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$p}mosques WHERE slug LIKE %s LIMIT 1", '%' . $wpdb->esc_like( $slug ) . '%' ) );
    if ( $id ) {
        $targets[ $id ] = $data;
        $wpdb->update( $p . 'mosques', $data, [ 'id' => $id ] );
        echo "Updated: {$data['name']} (ID $id, slug match: $slug)\n";
    } else {
        echo "SKIP: No mosque found matching slug '$slug'\n";
    }
}

// Always include the demo mosque
if ( ! isset( $targets[ $demo_id ] ) ) {
    $targets[ $demo_id ] = [ 'name' => 'YourNiyyah Masjid' ];
    echo "Using demo mosque ID $demo_id\n";
}

$mosque_ids = array_keys( $targets );

// Map old hardcoded IDs to current dynamic IDs
$id_list = array_values( $mosque_ids );
$id_map = [
    2   => $id_list[0] ?? $demo_id,
    337 => $id_list[1] ?? $demo_id,
    7   => $id_list[2] ?? $demo_id,
    222 => $id_list[3] ?? $demo_id,
    305 => $id_list[4] ?? $demo_id,
];
