...
class YNJ_UI {
    ...
    public static function render_admin_toolbar( $mosque_slug ) {
        ...
        <div class="ynj-admin-toolbar">
            <a href="<?php echo esc_url( home_url( '/mosque/' . $mosque_slug ) ); ?>" class="ynj-atb-primary">📢 <?php esc_html_e( 'New Post', 'yourjannah' ); ?></a>
            <a href="<?php echo esc_url( home_url( '/dashboard' ) ); ?>" class="ynj-atb-outline">📊 <?php esc_html_e( 'Dashboard', 'yourjannah' ); ?></a>
            <button type="button" onclick="var m=document.getElementById('<?php echo esc_js( $menu_id ); ?>');m.style.display=m.style.display==='block'?'none':'block'" class="ynj-atb-outline">⚡ <?php esc_html_e( 'Quick Menu', 'yourjannah' ); ?></button>
        </div>
        <div id="<?php echo esc_attr( $menu_id ); ?>" style="display:none;position:fixed;bottom:64px;left:12px;background:#fff;border:1px solid #e5e7eb;border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,.15);padding:12px;z-index:901;width:320px;max-width:90vw;">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
                <a href="<?php echo esc_url( home_url( '/dashboard/announcements' ) ); ?>" style="display:flex;align-items:center;gap:8px;padding:12px;background:#f9fafb;border-radius:10px;text-decoration:none;color:#333;font-size:12px;font-weight:600;">📢 Announcements</a>
